...
1600 & 840   & 78.7   & 0.0 \\
3200 & 3443  & 357.5  & 0.0 \\
6400 & 14354 & \textbf{1428.5} & \textbf{0.0} \\
\bottomrule
\end{tabular}
...
\smallskip
Tiempo: HB es $\sim 2.4\times$ m\'as lento.
...
\begin{frame}{Validaci\'on local: Biopython \texttt{PairwiseAligner}}
...
\begin{frame}{Comparaci\'on contra \textbf{BLAST} de NCBI}
La consigna pide ``comparar tu algoritmo con BLAST''. Procedimiento:
\begin{itemize}\setlength\itemsep{0.15em}
  \item Una llamada \texttt{blastp} online por especie (\texttt{Bio.Blast.NCBIWWW.qblast}, base \texttt{nr}), filtrada por \texttt{entrez\_query} a los 5 accessions de los subjects.
  \item Para cada par se extrae bit-score, E-value y \%identidad del mejor HSP.
  \item Resultados cacheados en \texttt{output/blast\_cache.json} (corridas posteriores son offline).
\end{itemize}

\smallskip
{\small\input{nw_vs_blast_compact.tex}}

\smallskip
\textbf{Lectura:} NW global y BLAST local concuerdan dentro de $\sim$1\% en identidad y todos los E-values son $\le 10^{-50}$. La se\~nal filogen\'etica est\'a en el marcador, no en el algoritmo.
\end{frame}

\begin{frame}{Dendrograma NW}
\centering
\includegraphics[width=0.95\linewidth]{dendrogram_nw.png}

\smallskip
\small UPGMA (izq.) y single linkage (der.). Clade primate colapsa a distancia 0; at\'un como outgroup.
\end{frame}

\begin{frame}{Dendrograma desde \textbf{BLAST}}
\centering
\includegraphics[width=0.95\linewidth]{dendrogram_blast.png}

\smallskip
\small Distancias derivadas del \%id de BLAST (no del NW). \alert{Misma topolog\'ia y misma anomal\'ia (Rat\'on, Gallina)} $\Rightarrow$ no es artefacto del algoritmo de alineamiento.
\end{frame}
...
\smallskip
\textbf{No es un bug del c\'odigo} (Biopython da exactamente las mismas distancias \emph{y} BLAST reproduce la misma topolog\'ia): es un problema del \emph{marcador molecular}.
\end{frame}
...
\begin{enumerate}\setlength\itemsep{0.5em}
  \item \textbf{Hirschberg = score \'optimo de NW con memoria $O(\min(m,n))$.}
        Para 6400 bases: 1.4 GiB $\to$ 0 MiB adicionales, costo $\sim 2.4\times$ en tiempo.
  ...
  \item \textbf{Doble validaci\'on:} 15/15 pares coinciden en score y \% identidad contra \texttt{Bio.Align.PairwiseAligner}; BLAST de NCBI confirma identidades equivalentes y reproduce la \emph{misma} topolog\'ia an\'omala.
  \item \textbf{Lectura central:} cambiar el \emph{algoritmo} (NW $\to$ Hirschberg $\to$ BLAST) no altera la respuesta biol\'ogica; cambiar el \emph{marcador} s\'i. El algoritmo asegura optimalidad respecto a una funci\'on objetivo; la interpretaci\'on biol\'ogica depende del marcador, la matriz y el modelo de distancia.
\end{enumerate}
\end{frame}
