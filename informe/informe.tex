...
\input{../code/output/benchmark.tex}
\label{tab:bench}
...
\noindent\textbf{Lectura.} Hirschberg es $\sim 2.4\times$ m\'as lento (constante derivada de la recursi\'on, en la teor\'ia es 2$\times$; el extra viene de la implementaci\'on en Python puro). En cambio, para 6400~bases NW consume \textbf{1.4~GiB} adicionales, mientras Hirschberg permanece dentro del baseline ($\Delta \approx 0$). El cruce de viabilidad ocurre alrededor de 3200~bases en mi laptop, pero el patr\'on es: NW cl\'asico es inutilizable a partir de unos pocos miles de bases en cualquier m\'aquina de uso general.
...
\input{../code/output/distance_matrix.tex}
\label{tab:dist}

\paragraph{Validaci\'on local contra \texttt{Bio.Align.PairwiseAligner}.}
...
\paragraph{Comparaci\'on contra BLAST de NCBI.}
Para responder directamente a la consigna (``compara tu algoritmo con Blast'') ejecutamos \texttt{blastp} online (\texttt{Bio.Blast.NCBIWWW.qblast} contra la base \texttt{nr}) con una llamada por especie como query, filtrando los hits a las accessions de los otros cinco subjects via \texttt{entrez\_query}. Los resultados (bit-score, E-value, \%identidad del mejor HSP) se cachean en \texttt{output/blast\_cache.json} para reproducibilidad. La Tabla~\ref{tab:nwvsblast} contrasta las 15 identidades NW (global) con las de BLAST (local, mejor HSP). Las diferencias son $\le 1\%$ porque el HSP cubre pr\'acticamente toda la prote\'ina ($\ge 95\%$ de query coverage) y los E-values son $\le 10^{-50}$ en todos los casos. La conclusi\'on metodol\'ogica es importante: para prote\'inas cortas y altamente conservadas como el citocromo c, BLAST local y NW global producen identidades equivalentes; las diferencias provienen \'unicamente del manejo de los bordes.

\input{../code/output/nw_vs_blast.tex}
\label{tab:nwvsblast}

\paragraph{Dendrograma NW.}
La Figura~\ref{fig:tree-nw} presenta dos topolog\'ias obtenidas a partir de la matriz NW: UPGMA (average linkage) y vecino m\'as cercano (single linkage). El cluster (Humano, Chimpanc\'e, Gorila) aparece como un \'unico nodo a distancia 0 en ambos m\'etodos (los tres son indistinguibles a nivel de citocromo c). El At\'un (\emph{Katsuwonus pelamis}) se posiciona como outgroup, consistente con la filogenia conocida vertebrado/pez.

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{dendrogram_nw.png}
...
\paragraph{Dendrograma BLAST y comparaci\'on de topolog\'ias.}
Se construy\'o adem\'as una segunda matriz de distancias usando las identidades de BLAST (Tabla embebida en la salida \texttt{distance\_matrix\_blast.tex}) y se reconstruy\'o un dendrograma an\'alogo (Figura~\ref{fig:tree-blast}). \textbf{El \'arbol derivado de BLAST reproduce la misma topolog\'ia y la misma anomal\'ia Rat\'on--Gallina} que el de NW: los primates se agrupan a distancia $\approx 0$, el at\'un queda como outgroup, y la rama mam\'iferos/aves se invierte de la misma manera. Esto es un control metodol\'ogico cr\'ucial: si la anomal\'ia desapareciera al cambiar de alineador, ser\'ia atribuible al algoritmo; el hecho de que persista identica al cambiar de NW global a BLAST local demuestra que la se\~nal err\'onea reside en el \emph{marcador molecular}, no en el m\'etodo de alineamiento.

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{dendrogram_blast.png}
\caption{Dendrogramas reconstruidos desde la matriz de distancias por BLAST (\texttt{blastp} contra \texttt{nr}, \%id del mejor HSP). Misma topolog\'ia y misma anomal\'ia Rat\'on--Gallina que el dendrograma NW (Fig.~\ref{fig:tree-nw}), evidenciando que el origen del error es el marcador.}
\label{fig:tree-blast}
\end{figure}

\paragraph{Concordancia con el \'arbol evolutivo conocido.}
El \'arbol esperado para estas seis especies es $((((Humano,Chimp,Gorila),Rat\'on), Gallina), At\'un)$: una jerarqu\'ia anidada (primates) $\subset$ mam\'iferos $\subset$ amniotas $\subset$ vertebrados con mand\'ibula. \textbf{Nuestro \'arbol coincide parcialmente.} Los primates s\'i se agrupan correctamente; el at\'un es correctamente el outgroup m\'as distante. Sin embargo, las distancias internas muestran una \emph{anomal\'ia}: $d(\text{Rat\'on}, \text{Gallina}) = 7.6$ es \emph{menor} que $d(\text{Humano}, \text{Rat\'on}) = 8.6$, lo que hace que UPGMA agrupe (Rat\'on, Gallina) antes que (Primates, Rat\'on). La topolog\'ia correcta unir\'ia los mam\'iferos primero (primates + rat\'on) y despu\'es la gallina. Como se discuti\'o, esta anomal\'ia aparece id\'entica con NW y con BLAST, confirmando que es del marcador.
...
(1)~El algoritmo de Hirschberg implementado sobre la jerarqu\'ia OO de la PC2 reproduce \emph{exactamente} el score \'optimo de NW con memoria $O(\min(m,n))$: para 6400~bases la reducci\'on es de 1.4~GiB $\to$ 0~MiB adicionales, a costa de $\sim 2.4\times$ en tiempo. (2)~La matriz de distancias por NW+BLOSUM62 sobre seis ort\'ologos de citocromo c agrupa correctamente los primates y separa el at\'un como outgroup, pero falla en la topolog\'ia interna mam\'iferos/aves por saturaci\'on y heterogeneidad de tasa del marcador. (3)~Doble validaci\'on independiente: los 15~pares dan id\'enticos score e identidad contra \texttt{Bio.Align.PairwiseAligner}, y las identidades de BLAST de NCBI (\texttt{blastp} contra \texttt{nr}) difieren menos de $1\%$ de las de NW; m\'as a\'un, el dendrograma reconstruido con distancias BLAST exhibe la \emph{misma} topolog\'ia err\'onea, descartando el algoritmo como fuente del problema. (4)~Cambiar el algoritmo (NW~$\to$~Hirschberg~$\to$~BLAST) no altera la respuesta biol\'ogica; cambiar el marcador s\'i. El algoritmo asegura optimalidad respecto a una funci\'on objetivo; la interpretaci\'on biol\'ogica depende del marcador, la matriz y el modelo de distancia.
